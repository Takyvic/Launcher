...
SolveSend(button, info) {
    ; Get the GUI from the button control
    parentGui := button.Gui
    
    ; Loop through ALL controls and find Edit controls
...
Solve(tries) {
	...
	for i in idx {
		...
		if(tries == 0) {
			...
			solveButton.OnEvent("Click", SolveSend)
...
generateClick(*) {
    Chars := "ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz_"
    Result := ""
